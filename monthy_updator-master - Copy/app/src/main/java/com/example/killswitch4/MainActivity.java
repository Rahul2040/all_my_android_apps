               ...
               /* InputStream is= null;
                ...
                noteref.orderBy("cusid").get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
                    ...
                    public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                        final int tot=queryDocumentSnapshots.size();
                        final cusinfo cus[]=new cusinfo[tot];
                        final String docids[]=new String[tot];
                        ...
                        altdial.setMessage("there are "+tot+" no of documents which are going to be affected. Do you want to proceed?").setCancelable(false)
                                .setPositiveButton("Proceed", new DialogInterface.OnClickListener() {
                                    ...
                                    public void onClick(DialogInterface dialog, int which) {
                                        ...
                                        for(int i=0;i<tot;i++){
                                         /* Log.e("e",cus[i].fetchcusinfo(1)+" docids  "+docids[i]);
                                             final String current_docid=docids[i];
                                            db.collection("List").document(docids[i]).delete().addOnSuccessListener(new OnSuccessListener<Void>() {
                                                @Override
                                                public void onSuccess(Void aVoid) {
                                                    Log.e("e","successfully deleted "+current_docid);
                                                }
                                            });*/
                                            ...
                                            int bal=total;
                                            final String current_id=cus[i].cusid;
                                            ...
                                            db.collection("List").document(docids[i])
                                                ...
                                                public void onSuccess(Void aVoid) {
                                                    ...
                                                    pb.setProgress(pb.getMin()+current_i/tot*(pb.getMax()-pb.getMin()));
                                                }
                                            });
                                        }
